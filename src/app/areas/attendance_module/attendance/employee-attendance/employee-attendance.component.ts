...
@Component({
  selector: 'attendance-module-employee-attendance-list',
...
export class EmployeeAttendanceComponent implements OnInit {
  ...
  getEmployeeAttendanceSummery(pageNo: number = 1) {
    this.attnSummeryPageNo = pageNo;
    let params = { employeeId: this.searchBySummeryEmployee, month: this.searchByMonth, year: this.searchByYear, pageNumber: this.attnSummeryPageNo, pageSize: this.attnSummeryPageSize };

    this.employeeAttendanceService.get(params).subscribe(response => {
  ...
  getEmployeeDailyAttendance(pageNo: number = 1) {
    ...
    this.employeeAttendanceService.getById({ employeeId: this.searchByAttnEmployee, fromDate: fromDate, toDate: toDate, pageNumber: this.dailyAttnPageNo, pageSize: this.dailyAttnPageSize })
      .subscribe(response => {
  ...
  downloadEmployeeDailyAttendanceReport(employeeId: number, month: number, year: number) {
    this.employeeAttendanceService.employeeReport({ employeeId: employeeId, attendanceMonth: month, attendanceYear: year }).subscribe(response => {
  ...
  downloadDailyAttendanceReport() {
    let fromDate = this.searchByDate.length == 0 ? "" : this.datepipe.transform(this.searchByDate[0], 'yyyy-MM-dd');
    let toDate = this.searchByToDate.length == 0 ? this.datepipe.transform(this.searchByDate[1], 'yyyy-MM-dd') : "";
    let params = { employeeId: this.searchByAttnEmployee == null ? 0 : this.searchByAttnEmployee, fromDate: fromDate, toDate: toDate};
    this.employeeAttendanceService.dateRangeReport(params).subscribe(response=>{
      this.utilityService.randerFile(response, 'application/pdf');
    }, (error) => {
