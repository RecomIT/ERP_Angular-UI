...
@Component({
  selector: 'attendance-module-manual-attendance',
...
export class ManualAttendanceComponent implements OnInit, AfterViewInit {
  ...
  openManualAttendanceModal() {
    ...
    // this.isEmployeeNgSelect2IntiInForm = false;
  ...
  submitManualAttendance() {
    if (this.manualAttendanceForm.valid) {
      ...
      this.manualAttendanceService.save(this.manualAttendanceForm.value).subscribe(response => {
        if (response.status) {
          this.manualAttendanceForm.reset;
  ...
  getManualAttendances() {
    ...
    let params = {
      ...
      toDate: toDate ?? '', status: this.searchByStatus, employeeId: this.utilityService.IntTryParse(this.searchByEmployee),
      pageSize: this.manualAttnPageSize, pageNumber: this.manualAttnPageNo
    };

    this.manualAttendanceService.get(params).subscribe(response => {
  ...
  getManualAttendance(id: number) {
    this.manualAttendanceService.getById({ manualAttendanceId: id }).subscribe(response => {
  ...
  confirmDelete() {
    if (Object.keys(this.manualAttnInfoItem).length > 0) {
      this.manualAttendanceService.delete({ manualAttendanceId: this.manualAttnInfoItem.manualAttendanceId, 
        employeeId: this.manualAttnInfoItem.employeeId }).subscribe(response => {
          if (response.status) {
            this.utilityService.success(response.msg, "Server Response")
            this.modalService.service.dismissAll();
            this.getManualAttendances()
