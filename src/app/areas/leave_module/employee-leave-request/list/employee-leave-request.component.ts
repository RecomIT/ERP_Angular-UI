...
export class EmployeeLeaveRequestComponent implements OnInit {
  ...
  loadEmployeesForSearch() {
    ...
    this.employeeInfoService.ddl_employee_data$.subscribe(data => {
      ...
      this.listOfEmployee = this.employeeInfoService.ddl$;
    }, error => {
  ...
  employeeLeaveTypeFormInit() {
    ...
    this.employeeLeaveTypeForm.get('employeeId').valueChanges.subscribe((value) => {
      setTimeout(() => {
        this.ddlEmployeeLeaveBalance = [];
        this.listOfeligibleLeaveDay = [];
        this.loadEmployeeLeaveBalance(value);
        this.getLeavePeriod(value);
      }, 5)
    })
    ...
    this.employeeLeaveTypeForm.get('leaveTypeId').valueChanges.subscribe((value) => {
      ...
      this.getLeaveSetting(this.utilityService.IntTryParse(this.employeeLeaveTypeForm.get('leaveTypeId').value), this.utilityService.IntTryParse(this.employeeLeaveTypeForm.get('employeeId').value));
    })
  ...
  leaveDaysCalculation() {
    ...
    if (this.employeeLeaveTypeForm != null && this.employeeLeaveTypeForm.get('appliedFromDate').value != null
      ...
      if (fromDate.indexOf('1970') < 0) {
        var params = {
          employeeLeaveRequestId: this.employeeLeaveTypeForm.get('employeeLeaveRequestId').value,
          employeeId: this.utilityService.IntTryParse(this.employeeLeaveTypeForm.get('employeeId').value),
          leaveTypeId: leaveTypeId, appliedFromDate: fromDate ?? "", appliedToDate: toDate ?? ""
        };
  ...
  loadEmployeeLeaveBalance(employeeId: number) {
    this.ddlEmployeeLeaveBalance = [];
    this.leaveBalanceService.loadEmployeeLeaveBalanceDropdown({ employeeId: employeeId });
  ...
  submitEmployeeLeaveRequest2() {
    if (this.employeeLeaveTypeForm.valid) {
      ...
      this.employeeLeaveRequestService.save2(requestData).subscribe(response => {
        this.btnEmployeeLeaveRequest = false;
        if (response.status) {
          response.employeeId = this.utilityService.IntTryParse(this.employeeLeaveTypeForm.get('employeeId').value);
  ...
  getEmployeeLeaveRequests() {
    ...
    var params = { employeeId: this.utilityService.IntTryParse(this.searchByEmployee), leaveTypeId: this.searchByLeaveType, dayLeaveType: this.searchByDayLeaveType ?? "", appliedFromDate: fromDate ?? "", appliedToDate: toDate ?? "", stateStatus: this.searchByStatus ?? "", pageNumber: this.employeeLeaveTypePageNo, pageSize: this.employeeLeaveTypePageSize };
  ...
  editEmployeeLeaveRequest(item: any) {
    this.employeeLeaveRequestService.getById({ employeeLeaveRequestId: item.employeeLeaveRequestId, employeeId: item.employeeId }).subscribe(response => {
      this.getLeavePeriod(item.employeeId)
    ...
    if (Object.keys(this.employeeDeleteItem).length > 0) {
      ...
      this.employeeLeaveRequestService.delete(params).subscribe(response => {
        ...
        if (result?.status) {
          ...
          response.employeeId = params.employeeId;
          this.sendEmail2(response)
          this.employeeDeleteItem = null;
          this.employeeLeaveTypePageNo = 1
          this.getEmployeeLeaveRequests();
  ...
  getLeavePeriod(employeeId: number) {
    this.leaveSettingSerive.getLeavePeriod({ employeeId: employeeId }).subscribe(response => {
  ...
  getLeaveSetting(leaveTypeId: number, employeeId: number) {
    if (leaveTypeId > 0 && employeeId > 0) {
      this.leaveSettingSerive.getLeaveTypeSetting({ leaveTypeId: leaveTypeId, employeeId: employeeId }).subscribe(response => {
        if (response != null) {
  ...
  sendEmail2(params: any) {
    ...
    this.employeeLeaveRequestService.sendEmail({ employeeId: params.employeeId, leaveTypeId: leaveTypeId, emailType: emailType, leaveRequestId: leaveRequestId }).subscribe(response => {
      console.log("response >>>", response);
    }, (error) => {
      console.log("error >>>", error);
      this.utilityService.fail("Something went wrong", "Server Response");
  ...
  showEmployeeLeaveRequestModal: boolean = false;
  leaveItem: any;
  request_id: number=0;
  openLeaveRequestModal(item: any) {
    this.showEmployeeLeaveRequestModal = true;
    this.leaveItem = item;
    this.request_id= item?.employeeLeaveRequestId;
  }

  closeLeaveRequestModal(reason: any) {
    this.showEmployeeLeaveRequestModal = false;
    if (reason == 'delete successful') {
      this.getEmployeeLeaveRequests();
    }
  }
