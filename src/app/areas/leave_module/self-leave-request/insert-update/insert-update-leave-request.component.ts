...
@Component({
    selector: 'leave-module-insert-update-leave-modal',
    templateUrl: './insert-update-leave-request.component.html',
    styleUrls: ['./insert-update-leave-request.component.css'],
    animations: [
        trigger('fadeInOut', [
            state('void', style({
                opacity: 0
...
export class InsertUpdateLeaveRequestModal implements OnInit {

    @Input() id: number = 0;
    @Input() leaveItem: any = null;

    @ViewChild('employeeLeaveRequestModal', { static: true }) employeeLeaveRequestModal!: ElementRef;
    ...
    ngOnInit(): void {
        // console.log("this.leaveItem >>>>>", this.leaveItem);
        // console.log("this.id >>>>>", this.id);

        if (this.leaveItem != null) {

            // console.log("this.dayLeaveTypeInEditMode >>>", this.dayLeaveTypeInEditMode);
            this.leaveTypeDisabled = true;
            this.modalTitle = "Update Your Leave Request";
            this.editEmployeeLeaveRequest(this.leaveItem);
        }
        else {
            this.openEmployeeLeaveRequestModal();
            this.modalTitle = "Apply New Leave Request"
        }
    ...
    loadEmployeeLeaveBalance() {
        ...
        if (this.utilityService.IntTryParse(this.leaveItem?.employeeLeaveRequestId) > 0) {
            this.leaveBalanceService.loadEmployeeLeaveBalanceDropdownInEdit({ employeeLeaveRequestId: this.leaveItem.employeeLeaveRequestId, employeeId: this.User().EmployeeId });
