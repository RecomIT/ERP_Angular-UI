...
import { uploaTransferProposalHeads } from 'src/models/hrm/employee-model';


@Component({
  selector: 'app-employee-module-upload-employee-transfer-proposal-modal',
  templateUrl: './upload-employee-transfer-proposal-modal.component.html'
})
export class UploadEmployeeTransferProposalModalComponent implements OnInit {
  @ViewChild('uploadEmployeeTransferProposalModal', { static: true }) uploadEmployeeTransferProposalModal!: ElementRef;
  @Input() transferProposalId: any = 0;
  ...
  formInit() {
    ...
    this.modalService.open(this.uploadEmployeeTransferProposalModal, "sm");
  }  
  ...
  uploaTransferProposalHead: uploaTransferProposalHeads = {  
    transferProposalId: 0,  
    employeeCode: '',   
    head: '',   
    proposalValue: '',   
    effectiveDate: null,
  ...
  clearUploadHeadObj() {
    this.excelFileName = "";
    this.uploaTransferProposalHead = {
      transferProposalId: 0,  
      employeeCode: '',   
      head: '',   
      proposalValue: '',   
  ...
  submitTransferProposalExcel() {
    if (this.uploadFormGroup.valid && this.excelFileName != "Choose Your excel file") {
      ...
      this.areasHttpService.observable_post<any>((ApiArea.hrms + ApiController.employees + "/UploadTransferProposal"),
    ...
    else if (this.fileExtension(this.uploaTransferProposalHead.excelFile.name) != 'xls' || this.fileExtension(this.uploaTransferProposalHead.excelFile.name) != 'xlsx') {
      this.utilityService.fail("Invalid File format")
    }
  }

  downloadTransferProposalExcelFile() {
    this.areasHttpService.observable_get<any>((ApiArea.hrms + ApiController.employees + "/DownloadTransferProposalExcelFile"), {
      responseType: 'blob', params: { fileName: "UploadEmpTransferProposalFile.xlsx" }
      ...
      if (response.size > 0) {
        ...
        link.download = 'Employee Transfer Proposal.xlsx';
        link.click();
