...
export class ClientUserConfigComponent implements OnInit {
  ...
  loadBranch() {
    this.ddlBranchWithCompanyAndOrganization = [];
    this.getBranches({ flag: "8", ComId: this.User().ComId, OrgId: this.User().OrgId }).subscribe(res => {
  ...
  createUser() {
    ...
    this.loadBranch();
    this.menusData = [];
    //this.loadRoleAuth(this.User().RoleId, this.User().ComId,this.User().OrgId);
  }
  ...
  loadEmployeeAndRole() {
    ...
    this.ddlRole = [];
    this.menusData = [];
    console.log("branchWithCompanyAndOrganization >>>", this.branchWithCompanyAndOrganization);
  ...
  loadUserInfoFromEmployee(event: any) {
    if (this.appUser.employeeId != null && this.appUser.employeeId > 0 && this.branchWithCompanyAndOrganization != "" && this.branchWithCompanyAndOrganization != null) {
      ...
      this.appUser.fullName = "";
      this.appUser.email = "";
      this.appUser.address = "";
  ...
  getApplicationUsers() {
    ...
    var params = { companyId: this.User().ComId, organizationId: this.User().OrgId };
  ...
  roleAuth() {
    ...
    this.userEditEnabled = false;
    this.loadRoles(this.User().OrgId, this.User().ComId);
    this.menusData = [];
  }
  ...
  getMenusForRolesAuth() {
    ...
    this.areasHttpService.observable_get((ApiArea.controlpanel + ApiController.administration + "/GetAppMenusForPermission"),
        {
          responseType: "json", params: {
            roleId: this.roleAuthRoleId, companyId: this.User().ComId, organizationId: this.User().OrgId, flag: "Role"
          }
        }).
        subscribe(data => {
          this.menusData = data as any[];
  ...
  getApplicationRoles() {
    this.areasHttpService.observable_get((ApiArea.controlpanel + ApiController.administration + "/GetApplicationRoles"),
      { params: { RoleName: '', RoleOrgId: this.User().OrgId, RoleComId: this.User().ComId } }).
      subscribe(data => {
  ...
  submitRoleAuth(roleAuthForm: NgForm) {
    if (roleAuthForm.valid) {
      ...
      this.areasHttpService.observable_post((ApiArea.controlpanel + ApiController.administration + "/SaveRoleAuth"), this.menusData, {
        params: {
          RoleId: this.roleAuthRoleId, BranchId: 0, CompanyId: this.User().ComId, OrganizationId: this.User().OrgId, UserId:
            this.User().UserId
        }
      }).subscribe((result) => {
        var data = result as any;
  ...
  loadRoleAuth(id: any) {
    ...
    this.roleAuthComId = this.User().ComId;
    this.roleAuthOrgId = this.User().OrgId;
    this.roleAuth();
