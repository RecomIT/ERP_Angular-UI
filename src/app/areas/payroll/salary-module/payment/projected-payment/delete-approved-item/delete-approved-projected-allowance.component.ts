...
@Component({
    selector: 'app-payroll-approved-pending-projected-allowance',
    templateUrl: './delete-approved-projected-allowance.component.html'
})

export class DeleteApprovedProjectedAllowanceComponent implements OnInit {
    @Input() id: number = 0;
    ...
    confirmDelete() {
        if (this.id > 0 && this.btnSubmit == false) {
            if (confirm("Are you sure you want to delete this item?")) {
                this.btnSubmit = true;
                this.service.deleteApprovedAllowance({ id: this.id }).subscribe({
                    next: (response) => {
