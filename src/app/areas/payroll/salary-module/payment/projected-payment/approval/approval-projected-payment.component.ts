...
@Component({
    selector: 'app-payroll-approval-projected-allowance',
    templateUrl: './approval-projected-payment.component.html'
})

export class ApprovalProjectedPaymentComponent implements OnInit {
    @Input() id: number = 0;
    ...
    approval(status: any) {
        if (this.id > 0 && this.btnSubmit == false) {
            if (confirm(`Are you sure you want to ${status} this item?`)) {
                this.btnSubmit = true;
                this.service.approval({ id: this.id, status: status }).subscribe({
                    next: (response) => {
