...
import { ProjectedPaymentService } from "../projected-payment/projected-payment.service";
...
export class UploadProjectedPaymentModalComponent implements OnInit {
    ...
    submit() {
        if (this.form.valid) {
            ...
            formData.append("fiscalYearId", this.form.get('fiscalYearId').value.toString());
            formData.append("file", this.form.get('file').value);

            this.projectedPaymentService.upload(formData).subscribe(response => {
                this.btnSubmit = false;
                //console.log("response >>>", response);               
                if(response[0]?.status){
                    this.utilityService.success(response[0]?.msg,"Server Response")
