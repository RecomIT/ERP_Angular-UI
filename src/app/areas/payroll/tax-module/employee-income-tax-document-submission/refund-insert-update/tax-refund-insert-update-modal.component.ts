...
@Component({
    selector: 'app-payroll-tax-refund-insert-update-modal',
    templateUrl: './tax-refund-insert-update-modal.component.html'
})
export class TaxRefundInsertUpdateModalComponent implements OnInit {

    @Input() id: any = 0;
    @Output() closeModalEvent = new EventEmitter<string>();

    @ViewChild('modal', { static: true }) aitModal!: ElementRef;
    modalTitle: string = "Add Employee Tax Refund";

    constructor(private fb: FormBuilder,
        public utilityService: UtilityService,
        public modalService: CustomModalService,
        private service: EmployeeTaxRefundService,
    ...
    employeeRefund: any = null;
    getEmployeeTaxDocumentById() {
        this.service.getById({ id: this.id }).subscribe(response => {
            this.employeeRefund = response.body;
            this.setFormValues();
        }, (error) => {
    ...
    setFormValues() {
        this.form.get('submissionId').setValue(this.employeeRefund?.submissionId);
        this.form.get('employeeId').setValue(this.employeeRefund?.employeeId);
        this.form.get('fiscalYearId').setValue(this.employeeRefund?.fiscalYearId);
        this.form.get('certificateType').setValue(this.employeeRefund?.certificateType);
        this.form.get('amount').setValue(this.employeeRefund?.amount);
        this.form.get('isAuction').setValue(this.employeeRefund?.isAuction);
        this.form.get('filePath').setValue(this.employeeRefund?.filePath);
    }
