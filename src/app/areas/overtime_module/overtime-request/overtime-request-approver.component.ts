...
@Component({
  selector: 'overtime-request-approver',
  templateUrl: './overtime-request-approver.component.html',
  animations: [
...
export class OvertimeRequestApproverComponent implements OnInit {
  ...
  generateEmployeeDropDownList(){
    this.employeeList = [];
    this.configurations.teamMembers.forEach((x)=>{this.employeeList.push({id : x.employeeId.toString(), text : x.name + " ("+x.employeeCode + ")"});});
  ...
  getAllRequest(status: string) {
    var request = this.areasHttpService.observable_get<OvertimeRequest[]>((ApiArea.payroll + ApiController.Overtime + "/GetAllRequest?status=" + status), {
  ...
  getConfigurations() {
    var request = this.areasHttpService.observable_get<OvertimeCreateRequest[]>((ApiArea.payroll + ApiController.Overtime + "/CreateRequestFromApprover"), {
      responseType: "json", observe: 'response', params: {}
    });
  ...
  overtimeRequestForm(form: NgForm) {
    ...
    this.buttonAction == "Submit" ?
      /// Save  
      this.areasHttpService.observable_post((ApiArea.payroll + ApiController.Overtime + "/SaveRequestFromApprover"),
