...
import { LeaveCommonDashboardRoutingService } from '../../../common-dashboard-routing/leave-common-dashboard-routing/leave-common-dashboard-routing.service';
import { DatePipe } from '@angular/common';
import { UserService } from 'src/app/shared/services/user.service';
import { SubordinatesService } from '../../supervisor/service/subordinates.service';
...
import { LeaveTypeSerive } from 'src/app/areas/leave_module/leave-type/leave-type.service';
import { LeaveSettingSerive } from 'src/app/areas/leave_module/leave-setting/leave-setting.service';
import { AreasHttpService } from 'src/app/areas/areas.http.service';
import { UtilityService } from 'src/app/shared/services/utility.service';
import { HrWebService } from 'src/app/shared/services/hr-web.service';
import { CustomModalService } from 'src/app/shared/services/custom-modal.service';
import { LeaveBalanceService } from 'src/app/areas/leave_module/leave-balance/leave-balance.service';
...
@Component({
  selector: 'app-my-leave-applied-records',
  templateUrl: './my-leave-applied-records.component.html',
  styleUrls: ['./my-leave-applied-records.component.css']
})
export class MyLeaveAppliedRecordsComponent implements OnInit {


  constructor(
    private leaveCommonDashboard: LeaveCommonDashboardRoutingService,
  ...
  myLeaveAppliedConfig: any = this.userService.pageConfigInit("data", this.pageSize, 1, 0);
  ...
  onPageSizeChange() {
    this.pageNumber = this.myLeaveAppliedConfig.currentPage;
    this.getMyLeaveRequests();
  }
  ...
  getMyLeaveRequests() {
    ...
    this.leaveCommonDashboard.getMyLeaveAppliedRecordsApi<any>(params).subscribe({
      next: (response) => {
        this.leaveRecorsList = response.body;



        var xPaginate = JSON.parse(response.headers.get('X-Pagination'));
        this.myLeaveAppliedConfig = this.userService.pageConfigInit("myLeaveAppliedConfigData", xPaginate.itemsPerPage, xPaginate.currentPage, xPaginate.totalItems);

        if (this.leaveRecorsList.length > 0) {
          this.totalEmployees = this.leaveRecorsList[0].totalRowCount;
        }
        else {
  ...
  getLeaveSetting(leaveTypeId: number) {
    if (leaveTypeId > 0) {
      this.leaveSettingSerive.getLeaveTypeSetting({ leaveTypeId: leaveTypeId, employeeId: this.User().EmployeeId }).subscribe(response => {
        if (response != null) {
          console.error(" leaveSettingSerive response >>>>", response);
  ...
  editleave: any;
  showLeaveRequestModal(item: any) {
