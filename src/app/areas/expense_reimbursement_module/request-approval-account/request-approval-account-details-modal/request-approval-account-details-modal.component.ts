...
@Component({
  selector: 'expense-reimbursement-module-request-approval-account-details-modal',
  templateUrl: './request-approval-account-details-modal.component.html',
  styleUrls: ['./request-approval-account-details-modal.component.css'],
  animations: [
...
export class RequestApprovalAccountDetailsModalComponent implements OnInit {

  modalTitle: string = "Expense Request Details Acc";
  ...
  ngOnInit(): void {
    ...
    console.log("showModal >>>", this.transactionType ,this.requestId, this.employeeId, this.stateStatus, this.spendMode);    
   
    this.getEmployeeProfileInfo();
    this.userRoll = this.User().RoleName;
    
    if (this.spendMode === "Advance") {
      this.getRequestDataAdvance();
    }
    if (this.spendMode === "Self") {
      this.getRequestDataReimbursement();
  ...
  getRequestDataAdvance() {
    ...
    this.requestApprovalSerive.getByAdvanceId(params).subscribe(response => {
  ...
  getRequestDataReimbursement() {
    ...
    console.log("params Data >>>", params);
    this.reimbursementSerive.getByReimbursementId(params).subscribe(response => {
      if (Array.isArray(response)) {
        this.requestList = response;
      } else {
        this.requestList = [response];
