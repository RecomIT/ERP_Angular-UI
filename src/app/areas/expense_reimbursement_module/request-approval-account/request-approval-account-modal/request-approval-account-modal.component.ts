...
@Component({
  selector: 'expense-reimbursement-module-request-approval-account-modal',
  templateUrl: './request-approval-account-modal.component.html',
  styleUrls: ['./request-approval-account-modal.component.css']
})
export class RequestApprovalAccountModalComponent implements OnInit {
  ...
  @Output() openDetailsModalEvent = new EventEmitter<{ transactionType: string, requestId: number, employeeId: number, stateStatus: string, spendMode: string }>();

  userRoll: any;
  modalTitle: string = "Expense Request Approval ACCC";
  ...
  constructor(
    ...
    private reimbursementSerive: ReimbursementSerive
  ) { }
  ...
  ngOnInit(): void {
    ...
    // console.log("InputStatus >>>", this.transactionType,this.paymentStatus,this.dashboardId);

    if (this.dashboardId === "AdvanceDashboard") {
      this.getRequestDataAdvance(0);
    }
    if (this.dashboardId === "ReimbursementDashboard") {
      this.getRequestDataReimbursement(0);
    }
  ...
  searchFormInit() {
    ...
    this.searchForm.get('status').valueChanges.subscribe((item) => {
      this.pageSize = 1;
      this.statusChange = 1;
      // this.getRequestDataAdvance(this.statusChange);
      if (this.dashboardId === "AdvanceDashboard") {
        this.getRequestDataAdvance(0);
      }
      if (this.dashboardId === "ReimbursementDashboard") {
  ...
  getRequestDataAdvance(statusChange: number) {
    ...
    this.requestApprovalSerive.getAdvance(params).subscribe(response => {
  ...
  getRequestDataReimbursement(statusChange: number) {
    // console.log("statusChange Data >>>", statusChange);  
    let params = {
      userId: this.User().EmployeeId,
      transactionType: this.transactionType.replace('Request', '').trim(),
    ...
    this.list = []
    this.list_loading_label = null
    this.reimbursementSerive.getReimbursement(params).subscribe(response => {
      if ((response.body as any[]).length > 0) {
        // console.log("Request Data >>>", response.body);
        this.list = response.body;
        let xPaginate = JSON.parse(response.headers.get('X-Pagination'));
  ...
  openRequestDetailsModal(transactionType: string, requestId: number, employeeId: number, stateStatus: string, spendMode: string) {
    ...
    this.openDetailsModalEvent.emit({ transactionType, requestId, employeeId, stateStatus, spendMode });

    // console.log('dashboardId being passed:', this.spendMode);
  }
